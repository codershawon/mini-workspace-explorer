...
import { getChildren, getDescendantIds } from "@/lib/workspace-utils";
import type { ItemType, WorkspaceItem } from "@/types/workspace";
...
export function FolderView() {
  const { state, createItem, renameItem, deleteItem } = useWorkspace();
  ...
  // tell the user exactly what will be removed
  function getDeleteMessage(item: WorkspaceItem): ReactNode {
    if (item.type === "file") {
      return (
        <>
          <strong className="text-gray-800">{item.name}</strong> will be permanently deleted.
          This can&apos;t be undone.
        </>
      );
    }

    const nestedCount = getDescendantIds(state.items, item.id).length;
    if (nestedCount === 0) {
      return (
        <>
          The empty folder <strong className="text-gray-800">{item.name}</strong> will be
          permanently deleted.
        </>
      );
    }
    ...
    return (
      <>
        The folder <strong className="text-gray-800">{item.name}</strong> and everything inside
        it ({formatItemCount(nestedCount)}) will be permanently deleted. This can&apos;t be undone.
      </>
    );
  }
  ...
  return (
    <div className="p-4 sm:p-6">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <Toolbar
          canEditFolder={!isRoot}
          onCreate={openCreate}
          onRenameFolder={() => setDialog({ kind: "rename", item: folder })}
          onDeleteFolder={() => setDialog({ kind: "delete", item: folder })}
        />
      </div>
      ...
      {dialog?.kind === "create" && (
        <NameDialog
          ...
          placeholder={
            dialog.itemType === "folder" ? "e.g. Projects" : "e.g. notes (.txt added automatically)"
          }
          submitLabel="Create"
      ...
      {dialog?.kind === "rename" && (
        <NameDialog
          title={dialog.item.type === "folder" ? "Rename folder" : "Rename file"}
          label="New name"
          submitLabel="Rename"
          initialValue={dialog.item.name}
          selectBeforeExtension={dialog.item.type === "file"}
          onSubmit={(name) => renameItem(dialog.item.id, name)}
          onClose={closeDialog}
        />
      )}
      ...
      {dialog?.kind === "delete" && (
        <ConfirmDialog
          title={dialog.item.type === "folder" ? "Delete folder?" : "Delete file?"}
          message={getDeleteMessage(dialog.item)}
          confirmLabel="Delete"
          onConfirm={() => deleteItem(dialog.item.id)}
          onClose={closeDialog}
        />
      )}
    </div>
